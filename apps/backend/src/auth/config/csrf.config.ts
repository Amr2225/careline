...
export const csrfConfig = (configService: ConfigService): DoubleCsrfConfigOptions => ({
    ...
    getSessionIdentifier: (req: Request) => req.cookies.refreshToken ?? req.cookies.patientRefreshToken ?? "anonymous", // Stable per-session value (staff or patient refresh cookie), not the access token
